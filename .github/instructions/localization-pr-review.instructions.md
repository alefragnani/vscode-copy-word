...
applyTo: "package.nls.*.json,l10n/bundle.l10n.*.json"
---
...
- ✅ Nested properties preserved (e.g., `"copy-word.commands.toggle.title"`)
...
1. **Missing keys** - Incomplete localization
   ```
   Missing keys: ["copy-word.commands.copy.title", "copy-word.commands.cut.title"]
   ```
...
3. **Broken placeholders** - Runtime errors possible
   ```
   English: "Open a file first to {0} text"
   Localized: "Abrir um arquivo antes para o texto" // Missing {0}
   ```
...
1. **Extra keys** - May indicate stale translations
   ```
   Extra keys not in base file: ["copy-word.commands.oldCommand.title"]
   ```

2. **Untranslated values** - Values identical to English
   ```
   "copy-word.commands.copy.title": "Copy"  // Appears untranslated
   ```
...
- `copy-word.commands.copyTwice.title`
- `copy-word.configuration.newSetting.description`
...
- `copy-word.commands.oldFeature.title`
...
- `copy-word.activitybar.title`: "Copy Word"
- `copy-word.commands.copy.title`: "Copy"

(Note: Some terms like "Copy Word" may be intentionally kept in English depending on localization guidelines)
```
...
1. **Product names** - May remain in English: "Copy Word", "VS Code"
2. **Technical terms** - Some terms may be transliterated rather than translated
3. **Command IDs** - Must never be translated: `command:copy-word.copy`
4. **Setting keys** - Must never be translated: `copy-word.useOriginalCopyBehavior`
